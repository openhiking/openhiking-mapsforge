...
$(MAP_HIKING_SYMBOLS_OSM_FP): $(MAP_ROUTES_PBF_FP) | __check_MAP
	$(ROUTEMAPPER) --start-node-id=$(SYMBOLS_START_ID) --prio=$(MAP_SYMBOL_PRIORITY_FILE) --lookup=$(MAP_SYMBOL_LOOKUP_FILE) --stacking=$(MAP_SYMBOL_STACKING_FILE) --pictogram=$(MAP_HIKING_SYMBOLS_OSM_FP) --trail=$(MAP_TRAIL_COLORS_OSC_FP) $(DEFAULT_COLOR_OPT) $(MAP_ROUTES_PBF_FP)
